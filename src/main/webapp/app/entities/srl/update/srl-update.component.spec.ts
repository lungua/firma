...
describe('Srl Management Update Component', () => {
  ...
  describe('ngOnInit', () => {
    it('Should call User query and add missing value', () => {
      ...
      const inregistratDe: IUser = { id: 32662 };
      srl.inregistratDe = inregistratDe;

      const userCollection: IUser[] = [{ id: 96756 }];
      jest.spyOn(userService, 'query').mockReturnValue(of(new HttpResponse({ body: userCollection })));
      const additionalUsers = [inregistratDe];
    ...
    it('Should update editForm', () => {
      const srl: ISrl = { id: 456 };
      const inregistratDe: IUser = { id: 6855 };
      srl.inregistratDe = inregistratDe;

      activatedRoute.data = of({ srl });
      comp.ngOnInit();

      expect(comp.usersSharedCollection).toContain(inregistratDe);
